...
import { cache, getMultipleAccounts, ParsedAccountBase } from './accounts';
...
export function MarketProvider({ children = null as any }): JSX.Element {
  ...
  useEffect(() => {
    ...
    const initalQuery = async (): Promise<void> => {
...
export const useMarkets = (): MarketsContextState => {
  const context = useContext(MarketsContext);
  return context as MarketsContextState;
};

export const useMidPriceInUSD = (
  mint: string,
): {
  price: number;
  isBase: boolean;
...
export const usePrecacheMarket = (): ((mints: string[]) => void) => {
  const context = useMarkets();
  return context.precacheMarkets;
};

const bbo = (bidsBook: Orderbook, asksBook: Orderbook): number => {
...
const getMidPrice = (marketAddress?: string, mintAddress?: string): number => {
  const SERUM_TOKEN = TOKEN_MINTS.find(
    (a) => a.address.toBase58() === mintAddress,
  );
