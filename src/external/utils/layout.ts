...
export const publicKey = (property = 'publicKey'): unknown => {
  ...
  publicKeyLayout.decode = (buffer: Buffer, offset: number): PublicKey => {
...
export const uint64 = (property = 'uint64'): unknown => {
  ...
  layout.decode = (buffer: Buffer, offset: number): any => {
    const data = _decode(buffer, offset);
    return new BN(
      [...data]
        .reverse()
  ...
  layout.encode = (num: BN, buffer: Buffer, offset: number): any => {
...
export const rustString = (property = 'string'): unknown => {
  ...
  rsl.decode = (buffer: Buffer, offset: number): any => {
    const data = _decode(buffer, offset);
    return data.chars.toString('utf8');
  };

  rsl.encode = (str: string, buffer: Buffer, offset: number): any => {
    const data = {
